function [AX, b, betaG, betaT, Gmax_node, line_idx_active] = ...
        generalized_LFR_v3_AC(casename, BS1, GS1, P_ref, use_tight_theta)
...
%  1. 加载索引常量（与原版完全相同）
% ====================================================================
[PQ, PV, REF, BUS_I, BUS_TYPE, PD, QD, GS_col, BS_col, BUS_AREA, VM, ...
...
% 节点导纳矩阵
[Ybus, ~, ~] = makeYbus(baseMVA, bus, branch_cur);
...
% ---------- 4b. 负荷侧等效灵敏度矩阵 Mat（[nl×nb]）--------------------
%  假设各负荷节点维持恒功率因数：Qd_i = pf_ratio_i * Pd_i
%  则：Pf_l = GSF_PP_F_l*(Pg-L) + GSF_PQ_F_l*(Qg - pf_ratio.*L)
%           = [...Gmax贡献...] - (GSF_PP_F + GSF_PQ_F.*pf_ratio')*L
%  等效负荷灵敏度矩阵：
%    Mat = GSF_PP_F + GSF_PQ_F .* pf_ratio'
Pd_bus = bus(:, PD) / baseMVA;   % 节点有功负荷 [nb×1]
Qd_bus = bus(:, QD) / baseMVA;   % 节点无功负荷 [nb×1]
...
% ---------- 4c. 发电侧等效灵敏度矩阵 Mat_G（[nl×nb]）------------------
%  发电机通常维持恒功率因数调度：Qg_i ≈ qpf_ratio_i * Pg_i
%  此处使用当前机组额定值估算，可近似为系统平均功率因数
Qgmax_unit = gen_cur(:, GEN_QMAX) / baseMVA;
qpf_ratio  = zeros(nb, 1);
for i = 1:ng
    k = unit_bus(i);
    if Pmax_unit(i) > 1e-6
        qpf_ratio(k) = qpf_ratio(k) + Qgmax_unit(i) / Pmax_unit(i) * 0.5;
        % 0.5系数：额定Qmax通常在0.5-1倍Pmax，取保守估计
    end
end

Mat_G = GSF_PP_F + GSF_PQ_F .* qpf_ratio';   % [nl×nb]，发电侧等效灵敏度
